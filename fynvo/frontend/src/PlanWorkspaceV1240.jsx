...
const monthLabel = (value) =>
  dateLabel(value, { month: "long", year: "numeric" });
...
const eventDate = (row) =>
  dateKey(row?.date || row?.expected_date || row?.due_date);
const eventAmount = (row) =>
  finite(row?.amount ?? row?.expected_amount ?? row?.estimated_amount) ?? 0;
const forecastPoints = (forecast) =>
  Array.isArray(forecast?.chart_points)
    ? forecast.chart_points.filter(
        (point) =>
          finite(point?.balance ?? point?.forecast_balance ?? point?.value) !==
          null,
      )
    : [];
const pointBalance = (point) =>
  finite(point?.balance ?? point?.forecast_balance ?? point?.value);
const eventType = (row) =>
  row?.direction === "income" || Number(eventAmount(row)) > 0
    ? "income"
    : row?.status === "overdue" || row?.requires_action
      ? "attention"
      : "payment";
...
function forecastSummary(forecast) {
  ...
  const lowest =
    forecast.lowest_balance && typeof forecast.lowest_balance === "object"
  ...
  return {
    ...
    lowest:
      finite(lowest?.balance) ??
      finite(forecast.lowest_balance) ??
      (points.length ? Math.min(...points.map(pointBalance)) : null),
...
function chartGeometry(forecast) {
  ...
  const values = points.map(pointBalance);
  ...
  const point = (row, index) => ({
    ...
    y:
      height -
      padY -
      ((pointBalance(row) - min) / spread) * (height - padY * 2),
  });
  ...
  const path = coords
    ...
    .join(" ");
  const zeroY = height - padY - ((0 - min) / spread) * (height - padY * 2);
...
function EventRow({ row, onOpen = () => {} }) {
  ...
  return (
    <button
      type="button"
      className={`fynvo-plan-v1240-event ${type}`}
      onClick={() => onOpen(row)}
    >
      <span className="fynvo-plan-v1240-event-icon">
        {incoming ? "↑" : type === "attention" ? "!" : "↓"}
      </span>
      <span className="fynvo-plan-v1240-event-copy">
        <strong>{row.name || row.merchant || "Financial event"}</strong>
        <small>
          {dateLabel(eventDate(row))}
          {row.explanation ? ` · ${row.explanation}` : ""}
        </small>
      </span>
      <strong className="fynvo-plan-v1240-event-amount">
        {incoming ? "+" : ""}
        {money(Math.abs(eventAmount(row)))}
      </strong>
    </button>
  );
}
...
function BalanceChart({ forecast, detailed = false }) {
  const geometry = chartGeometry(forecast);
  if (!geometry)
    return (
      <div className="fynvo-plan-v1240-chart-empty">
        No forecast data is available for this period.
      </div>
    );
  const labels = [
    geometry.points[0],
  ...
  return (
    <div
      className={`fynvo-plan-v1240-chart ${detailed ? "detailed" : ""}`}
      role="img"
      aria-label={`Projected balance from ${dateLabel(forecast.start_date)} to ${dateLabel(forecast.end_date)}. Lowest projected balance ${money(forecast.lowest_balance?.balance ?? forecast.lowest_balance)}.`}
    >
      <svg
      ...
      >
        <line
          ...
          y2={geometry.zeroY}
        />
        <path className="trajectory" d={geometry.path} />
        <circle
          className="lowest"
          cx={geometry.lowest.x}
          cy={geometry.lowest.y}
          r="5"
        />
      </svg>
      <div className="fynvo-plan-v1240-chart-labels">
        {labels.map((point, index) => (
          <span key={`${point.date}-${index}`}>
            {dateLabel(point.date, { day: "numeric", month: "short" })}
            <strong>{money(pointBalance(point))}</strong>
          </span>
        ))}
      </div>
      <div className="fynvo-plan-v1240-chart-legend">
        <span>
          <i className="trajectory" />
          Projected balance
        </span>
        <span>
          <i className="income" />
...
function CalendarGrid({ month, events, selectedDate, onSelect }) {
  ...
  return (
    <div
      className="fynvo-plan-v1240-calendar-grid"
      role="grid"
        ...
        {cells.map((date, index) => {
          ...
          return (
            <button
              ...
              aria-label={`${dateLabel(key, { weekday: "long", day: "numeric", month: "long" })}, ${rows.length} event${rows.length === 1 ? "" : "s"}`}
              onClick={() => onSelect(key)}
            >
              <strong>{date.getDate()}</strong>
              {rows.length > 0 && (
                <span className="markers">
                  {[...new Set(rows.map(eventType))].slice(0, 3).map((type) => (
...
export default function PlanWorkspaceV1240({ onNavigate = () => {} }) {
  const [view, setView] = useState(
    () => localStorage.getItem("fynvo.plan.view.v1240") || "overview",
  );
  ...
  const selectedEvents = calendarEvents.filter(
    (row) => eventDate(row) === selectedCalendarDate,
  );
  const pressure =
    expected?.shortfall ||
    (summary.lowest !== null && summary.lowest < 0
      ? {
          date: summary.lowestDate,
          balance: summary.lowest,
          name: "Projected shortfall",
        }
      : null);
  const incomeEvents = (expected?.events || [])
    .filter((row) => eventType(row) === "income")
    .slice(0, 3);
  const keyDates = [...(expected?.events || [])]
    .filter(
      (row) =>
        eventType(row) === "income" ||
        eventType(row) === "attention" ||
        Math.abs(eventAmount(row)) >=
          Math.max(1, (summary.expenses || 0) * 0.2),
    )
    .slice(0, 5);
  ...
  return (
    <section className="fynvo-plan-v1240" aria-label="Plan">
      <header className="fynvo-plan-v1240-heading">
        ...
        <h1>Plan</h1>
        <p>See ahead. Be prepared.</p>
      </header>
      <nav className="fynvo-plan-v1240-tabs" aria-label="Plan views">
        <button
          type="button"
        ...
        <button
          type="button"
          className={view === "allocation" ? "active" : ""}
          aria-pressed={view === "allocation"}
          onClick={() => selectView("allocation")}
        >
          Allocation
        </button>
        <button
      ...
      {error && (
        <div className="fynvo-plan-v1240-error" role="alert">
          <span>{error}</span>
          <button type="button" onClick={() => load(horizon)}>
            Retry
          </button>
        </div>
      )}
      {loading && !forecast ? (
        <div
          className="fynvo-plan-v1240-loading"
          role="status"
        ...
        <>
          {view === "overview" && (
            <>
              <section className="fynvo-plan-v1240-hero">
                <div>
                  <small>
                    Projected balance in{" "}
                    {HORIZONS.find((item) => item.days === horizon)?.label ||
                      `${horizon} days`}
                  </small>
                  <strong>{money(summary.ending)}</strong>
                  <p>Based on upcoming income and payments</p>
                </div>
                <span className={pressure ? "warning" : "healthy"}>
                  {pressure ? "Attention" : "On track"}
                </span>
              </section>
              <section className="fynvo-plan-v1240-card">
                <div className="fynvo-plan-v1240-section-head">
                ...
                <BalanceChart forecast={expected} />
              </section>
              <SummaryMetrics safe={safe} summary={summary} />
              {pressure ? (
                <section className="fynvo-plan-v1240-pressure">
                  <strong>Upcoming pressure point</strong>
                  <p>Your balance may be low soon.</p>
                  <div>
                    <span>
                      <b>{dateLabel(pressure.date)}</b>
                      <small>Projected date</small>
                    </span>
                    <span>
                      <b>{money(pressure.balance)}</b>
                      <small>Projected balance</small>
                    </span>
                  </div>
                </section>
              ) : (
                <section className="fynvo-plan-v1240-healthy">
                  <strong>Healthy trajectory</strong>
                  <p>No cash shortfall is predicted in this planning period.</p>
                </section>
              )}
              <section className="fynvo-plan-v1240-card">
                <div className="fynvo-plan-v1240-section-head">
                  <div>
                    <h2>Incoming income</h2>
                    <small>Next {incomeEvents.length || 0} events</small>
                  </div>
                ...
                {incomeEvents.length ? (
                  incomeEvents.map((row, index) => (
                    <EventRow
                      row={row}
                      key={`${eventDate(row)}-${row.name}-${index}`}
                    />
                  ))
                ) : (
                  <p className="fynvo-plan-v1240-state">
                    No upcoming income is available.
                  </p>
                )}
              </section>
            </>
          ...
          {view === "allocation" && (
            <>
              <section className="fynvo-plan-v1240-hero fynvo-plan-v1240-allocation-hero">
                <div>
                  <small>After payday, allocate</small>
                  <strong>
                    {money(allocation?.total_recommended_allocation)}
                  </strong>
                  <p>
              ...
              <section className="fynvo-plan-v1240-card fynvo-plan-v1240-allocation-list">
                <div className="fynvo-plan-v1240-section-head">
                  <div>
                    <h2>Payday allocation</h2>
                    <small>Recommended transfers by account</small>
                  </div>
                  <button
                    type="button"
                    className="link"
                ...
                {allocation?.accounts?.length ? (
                  allocation.accounts.map((row) => (
                    <div
                      className="fynvo-plan-v1240-allocation-row"
                      key={row.account_id}
                    >
                      <span>
                        <strong>{row.account_name}</strong>
                        <small>
                          {row.commitment_count
                            ? `${row.commitment_count} payment${row.commitment_count === 1 ? "" : "s"} · Target ${money(row.target_balance)}`
                            : "No payments due"}
                        </small>
                      </span>
                      <span>
                        <strong>
                          {row.status === "transfer"
                            ? `Transfer ${money(row.recommended_transfer)}`
                            : row.status === "already_funded"
                              ? "Already funded"
                        ...
                        <small>
                          Projected at payday{" "}
                          {money(row.projected_starting_balance)}
                        </small>
                      </span>
                    </div>
                  ))
                ...
                {allocation?.unassigned?.commitment_count > 0 && (
                  <button
                    type="button"
                    className="fynvo-plan-v1240-unassigned"
                    onClick={navigatePayments}
                  >
                    <strong>
                      {money(allocation.unassigned.required)} needs a funding
                      account
                    </strong>
                    <span>
                      {allocation.unassigned.commitment_count} unassigned
                      payment
                      {allocation.unassigned.commitment_count === 1 ? "" : "s"}{" "}
                      · Review payments ›
                    </span>
                  </button>
                )}
            ...
            <>
              <section className="fynvo-plan-v1240-card">
                <div className="fynvo-plan-v1240-section-head">
                  <div>
                    ...
                    <p>See your projected cash flow over time.</p>
                  </div>
                  <select
              ...
              <section className="fynvo-plan-v1240-card">
                <div className="fynvo-plan-v1240-section-head">
                  <div>
                    <h2>Key dates</h2>
                    <small>Important events in this forecast</small>
                  </div>
                  {keyDates.length > 0 && (
                    <button
                      type="button"
          ...
          {view === "calendar" && (
            <>
              <section className="fynvo-plan-v1240-card">
                <div className="fynvo-plan-v1240-calendar-head">
                  <button
                    type="button"
              ...
              <section className="fynvo-plan-v1240-card">
                <div className="fynvo-plan-v1240-section-head">
                  <h2>
                    {dateLabel(selectedCalendarDate, {
                      weekday: "short",
                      day: "numeric",
                      month: "short",
                      year: "numeric",
                  ...
                  selectedEvents.map((row, index) => (
                    <EventRow
                      ...
                      onOpen={
                        ...
                        row.source_type === "recurring_expense"
                          ? navigatePayments
                          : undefined
                      }
                    />
