...
import PaymentCentreMobileV1182 from './PaymentCentreMobileV1182.jsx';
...
export default function AppCorrectiveV0174({ authState = null }) {
  ...
  const [data, setData] = useState({ accounts: [], cards: [], transactions: [], income: [], recurring: [], scheduledPayments: [], paymentAttention: [], bills: [], planned: [], categories: [], expenseTypes: [], budgets: [], goals: [], imports: [], review: [], suggestions: [], insights: [], financialHealth: null, budgetAnalysis: null, forecast: null, command: null, paymentPlanning: null, forecastCache: {} });
  ...
  async function loadSupportingData() {
    ...
    setData((current) => {
      ...
      const cachedForecast = forecast ? { ...(current.forecastCache || {}), [rangeDays]: { expected: forecast, baseline: current.forecastCache?.[rangeDays]?.baseline } } : current.forecastCache;
      return {
        ...current,
        accounts: accounts || [], cards: cards || [], transactions: transactions || [], income: income || [], recurring: recurring || [], scheduledPayments: resolvedScheduled, bills: bills || [], planned: planned || [], categories: categories || [], expenseTypes: referenceData?.expense_types || [], budgets: budgets || [], goals: goals || [], imports: imports || [], review: review || [], suggestions: suggestions || [], insights: insights || [], financialHealth, budgetAnalysis, forecast, paymentPlanning: paymentPlanning === null ? current.paymentPlanning : paymentPlanning,
        paymentAttention: derivedAttention || [], forecastCache: cachedForecast || {},
      };
    });
  }
  async function loadAll() { await Promise.all([refreshDashboard(), loadSupportingData()]); }
  async function refreshRecurringSlice() {
    const [recurring, scheduledPayments] = await Promise.all([j('/recurring-expenses'), j('/scheduled-payments')]);
    setData((current) => ({ ...current, recurring: recurring || current.recurring, scheduledPayments: scheduledPayments || current.scheduledPayments, paymentAttention: (scheduledPayments || current.scheduledPayments || []).filter((row) => ATTENTION_STATUSES.has(row.status)) }));
    return { scheduleError: scheduledPayments === null };
  }
  ...
  useEffect(() => {
    if (active === 'Payment Centre') document.body.classList.add('fynvo-payment-centre-page');
    else document.body.classList.remove('fynvo-payment-centre-page');
    return () => document.body.classList.remove('fynvo-payment-centre-page');
  }, [active]);
  useEffect(() => { if (authState) { setAuth(authState); return; } if (!auth) loadAuth(); }, [authState]);
  useEffect(() => { if (!auth?.authenticated || initialLoadedRef.current) return; initialLoadedRef.current = true; loadAll(); }, [auth?.authenticated]);
  useEffect(() => { if (auth?.authenticated) refreshDashboard(); }, [rangeDays]);
  useEffect(() => { localStorage.setItem('fynvo.view', active); }, [active]);
  useEffect(() => { localStorage.setItem('fynvo.rangeDays', rangeDays); }, [rangeDays]);
  useEffect(() => { const timer = window.setInterval(() => setGreeting(greetingForNow()), 60000); return () => window.clearInterval(timer); }, []);

  async function submitAuth(e) { e.preventDefault(); setError(''); const path = auth?.setup_required ? '/auth/setup' : '/auth/login'; const res = await api(path, { method: 'POST', body: JSON.stringify(form) }); if (res.ok) await loadAuth(); else setError((await res.json().catch(() => null))?.detail || 'Could not sign in.'); }
  async function logout() { await api('/auth/logout', { method: 'POST' }); await loadAuth(); }

  async function saveEdit(e) {
    e.preventDefault(); setError(''); setSuccess('');
    const creating = !edit.row?.id;
    try {
      ...
      const payload = await apiRequest(path, { method, body: JSON.stringify(values) });
  ...
  async function createRecord(type, values) {
    setError(''); setSuccess('');
    setData((current) => ({ ...current, forecastCache: {} }));
    if (type === 'recurring') {
      try {
  ...
  async function commitImport() { const res = await api('/imports/commit', { method: 'POST', body: JSON.stringify(importState) }); if (res.ok) { setImportState({ ...importState, preview: await res.json() }); setData((current) => ({ ...current, forecastCache: {} })); await loadAll(); } else setError('CSV import failed. Review invalid rows and duplicates.'); }
  async function acceptMatch(id) { const res = await api(`/reconciliation/${id}/accept`, { method: 'POST' }); if (res.ok) { setData((current) => ({ ...current, forecastCache: {} })); await loadAll(); } else setError('Could not accept match.'); }
  ...
  const navigate = (item) => { if (item === 'Cards') { localStorage.setItem('fynvo.accountsView', 'cards'); setActive('Accounts'); } else { if (item === 'Accounts') localStorage.setItem('fynvo.accountsView', 'accounts'); setActive(item); } if (isMobile) { setMobileNavOpen(false); window.requestAnimationFrame(() => window.scrollTo({ top: 0, left: 0, behavior: 'auto' })); } else window.requestAnimationFrame(() => window.scrollTo({ top: 0, left: 0, behavior: 'auto' })); };
  ...
  const cacheForRange = data.forecastCache?.[rangeDays] || data.command?.forecast || (data.forecast ? { expected: data.forecast } : null);
  const rememberForecast = ({ baseline, expected, rangeDays: days }) => setData((current) => ({ ...current, forecastCache: { ...(current.forecastCache || {}), [days]: { baseline, expected } } }));
...
function Overview({ data, setActive, rangeDays, setQuick, quickDefaults }) {
  ...
  return <div className="dashboard-page">
    ...
    <section className="dashboard-secondary-row">
      ...
      <article className="panel dashboard-compact-panel dashboard-forecast-summary dashboard-drilldown-panel" role="button" tabIndex="0" onClick={() => setActive('Cash Flow')} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setActive('Cash Flow'); } }}><PanelHead title="Forecast Summary" meta={`End of ${rangeDays} days`}/><div className="list-row"><span>Baseline Forecast</span><strong>{money(baselineEnd) || '—'}</strong></div><div className="list-row"><span>Expected Forecast</span><strong>{money(expectedEnd) || '—'}</strong></div><div className="list-row"><span>Lowest Balance</span><strong>{money(lowest) || '—'}</strong></div><span className="link-button">View cash flow →</span></article>
      <article className="panel dashboard-compact-panel dashboard-forecast-summary dashboard-drilldown-panel" role="button" tabIndex="0" onClick={() => setActive('Payment Centre')} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setActive('Payment Centre'); } }}><PanelHead title="Money Needed Soon" meta="Authoritative unresolved obligations"/><div className="list-row"><span>Next 7 days</span><strong>{money(paymentPlanning.money_needed_soon?.next_7_days) || '—'}</strong></div><div className="list-row"><span>Next 30 days</span><strong>{money(paymentPlanning.money_needed_soon?.next_30_days) || '—'}</strong></div><div className="list-row"><span>Payments requiring attention</span><strong>{attentionCount}</strong></div><div className="list-row"><span>Next payment</span><strong>{nextPayment ? `${nextPayment.name} · ${money(nextPayment.expected_amount ?? nextPayment.amount) || 'Not set'} · ${dateLabel(nextPayment.expected_date || nextPayment.due_date)}` : '—'}</strong></div><span className="link-button">View All Payments →</span></article>
    </section>
    <section className="dashboard-main-row"><article className="panel dashboard-forecast-panel dashboard-drilldown-panel" role="button" tabIndex="0" onClick={() => setActive('Cash Flow')} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setActive('Cash Flow'); } }}><div className="panel-head compact"><h2>Cash Flow Forecast</h2><span className="link-button">View full cash flow →</span></div><CashFlowChartV0174 baseline={forecast} expected={expected} dateLabel={dateLabel} Empty={Empty}/></article><div className="dashboard-side-stack"><article className="panel dashboard-compact-panel dashboard-drilldown-panel" role="button" tabIndex="0" onClick={() => setActive('Payment Centre')} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setActive('Payment Centre'); } }}><div className="panel-head compact"><h2>Upcoming Commitments</h2><span className="link-button">Open Payment Centre →</span></div>{upcoming.length ? upcoming.map((row, index) => <div className="list-row" key={`${row.source_type || row.kind || 'payment'}-${row.id || row.source_id || index}-${row.date || row.expected_date || index}`}><span>{row.name}<small>{dateLabel(row.expected_date || row.due_date || row.date)} · {row.category || row.source_type?.replaceAll('_', ' ') || row.kind || 'payment'}</small></span><strong>{money(row.expected_amount ?? row.amount)}</strong></div>) : planned.length ? planned.slice(0, 7).map((row) => <div className="list-row" key={`${row.source_type || 'planned'}-${row.id || row.source_id}`}><span>{row.name}<small>{row.date || row.planned_date ? dateLabel(row.date || row.planned_date) : 'No date'}</small></span><strong>{money(row.amount || row.estimated_amount)}</strong></div>) : <Empty title="Nothing planned">Add recurring expenses, bills or planned spending to see commitments here.</Empty>}</article></div></section>
  </div>;
}
function Kpi({ icon, label, value, sub, onActivate }) { const content = <><span className="kpi-icon">{icon}</span><div><small>{label}</small><strong>{value ?? '—'}</strong><p>{sub}</p></div></>; return onActivate ? <button type="button" className="kpi kpi-link" onClick={onActivate} aria-label={`${label}: ${value ?? 'Not available'}`}>{content}</button> : <article className="kpi">{content}</article>; }
...
function CalendarPage({ command }) { const events = command?.calendar || command?.upcoming_commitments || command?.forecast?.expected?.events || command?.forecast?.baseline?.events || []; const [selectedDate, setSelectedDate] = useState(null); const grouped = events.reduce((map, event) => { const key = String(event.date || '').slice(0, 10) || 'No date'; if (!map.has(key)) map.set(key, []); map.get(key).push(event); return map; }, new Map()); const keys = [...grouped.keys()].sort(); const chosen = selectedDate || keys[0] || null; const chosenRows = chosen ? grouped.get(chosen) || [] : []; return <section className="panel calendar-v1162"><div className="panel-head"><div><h2>Calendar</h2><p className="muted">What is happening and when across your household finances.</p></div><small>{events.length} events</small></div>{keys.length ? <div className="calendar-v1162-layout"><div className="calendar-v1162-dates" role="list" aria-label="Financial event dates">{keys.map((key) => <button type="button" key={key} className={chosen === key ? 'active' : ''} onClick={() => setSelectedDate(key)}><strong>{key === 'No date' ? 'No date' : dateLabel(key)}</strong><small>{grouped.get(key)?.length || 0} event{(grouped.get(key)?.length || 0) === 1 ? '' : 's'}</small></button>)}</div><div className="calendar-v1162-day"><h3>{chosen === 'No date' ? 'No date' : dateLabel(chosen)}</h3>{chosenRows.map((event, index) => <div className="list-row" key={`${event.source_type || event.kind}-${event.source_id || index}-${event.date}-${index}`}><span>{event.name}<small>{(event.source_type || event.kind || 'financial event').replaceAll('_', ' ')}</small></span><strong>{money(event.amount)}</strong></div>)}</div></div> : <Empty title="Nothing scheduled">Financial events will appear here.</Empty>}</section>; }
...
function GoalsPage({ goals, onEdit, onAdd, onComplete }) { return <section className="panel"><PanelHead title="Goals" meta={`${goals.length} goals`}/><button className="primary" onClick={onAdd}>+ Add Goal</button>{goals.length ? goals.map((row) => <div className="list-row" key={row.id}><span>{row.name}<small>{row.status}</small></span><strong>{money(row.current_amount)} / {money(row.target_amount)}</strong><button onClick={() => onEdit(row)}>Edit</button>{row.status !== 'completed' && <button onClick={() => onComplete(row.id)}>Complete</button>}</div>) : <Empty title="No goals yet">Create a savings or debt goal.</Empty>}</section>; }
function CsvImport({ state, setState, accounts, previewImport, commitImport }) { return <section className="panel"><PanelHead title="CSV Import" meta="Australian bank CSV"/><form onSubmit={previewImport}><Field label="Account"><select value={state.account_id} onChange={(e) => setState({ ...state, account_id: e.target.value })}><option value="">Choose Account</option>{accounts.map((a) => <option value={a.id} key={a.id}>{a.name}</option>)}</select></Field><Field label="CSV text"><textarea rows="12" value={state.csv_text} onChange={(e) => setState({ ...state, csv_text: e.target.value })}/></Field><button className="primary">Preview</button></form>{state.preview && <><pre>{JSON.stringify(state.preview, null, 2)}</pre><button className="primary" onClick={commitImport}>Import</button></>}</section>; }
function ImportHistory({ rows }) { return <section className="panel"><PanelHead title="Import History" meta={`${rows.length} imports`}/>{rows.length ? rows.map((row) => <div className="list-row" key={row.id}><span>{row.filename || row.source_name}</span><strong>{row.created_count ?? row.row_count ?? 0}</strong></div>) : <Empty title="No imports yet">Imported files will appear here.</Empty>}</section>; }
function ReviewQueue({ rows, acceptMatch }) { return <section className="panel"><PanelHead title="Review Queue" meta={`${rows.length} items`}/>{rows.length ? rows.map((row) => <div className="list-row" key={row.id}><span>{row.description || row.merchant}<small>{row.reason}</small></span><button className="primary" onClick={() => acceptMatch(row.id)}>Accept</button></div>) : <Empty title="Nothing to review">Potential matches will appear here.</Empty>}</section>; }
function QuickAddModal({ onClose, onChoose }) { return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal quick-add-modal" role="dialog" aria-modal="true" aria-label="Quick Add"><div className="modal-head"><div><h2>Quick Add</h2><p>Choose what you want to add.</p></div><button type="button" onClick={onClose}>×</button></div><div className="quick-add-grid">{quickAddOptions.map(([type, label, description]) => <button key={type} type="button" onClick={() => onChoose(type)}><strong>{label}</strong><small>{description}</small></button>)}</div></section></div>; }
function EditModal({ edit, setEdit, onSubmit, data }) { const values = edit.values; const update = (key, value) => setEdit({ ...edit, values: { ...values, [key]: value } }); return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setEdit(null)}><form className="modal" onSubmit={onSubmit}><div className="modal-head"><div><h2>{edit.label}</h2></div><button type="button" onClick={() => setEdit(null)}>×</button></div><div className="form-grid">{edit.type === 'accounts' && <><Field label="Name" value={values.name} onChange={(e) => update('name', e.target.value)}/><Field label="Account type"><select value={values.account_type} onChange={(e) => update('account_type', e.target.value)}>{accountTypeOptions.map(([value,label]) => <option value={value} key={value}>{label}</option>)}</select></Field><Field label="Institution" value={values.institution} onChange={(e) => update('institution', e.target.value)}/><Field label="Opening balance" value={values.opening_balance} onChange={(e) => update('opening_balance', e.target.value)}/></>}{edit.type === 'transactions' && <><Field label="Account"><select value={values.account_id} onChange={(e) => update('account_id', e.target.value)}>{data.accounts.map((a) => <option value={a.id} key={a.id}>{a.name}</option>)}</select></Field><Field label="Date" type="date" value={values.date} onChange={(e) => update('date', e.target.value)}/><Field label="Amount" value={values.amount} onChange={(e) => update('amount', e.target.value)}/><Field label="Description" value={values.description} onChange={(e) => update('description', e.target.value)}/></>}{edit.type === 'income' && <><Field label="Name" value={values.name} onChange={(e) => update('name', e.target.value)}/><Field label="Amount" value={values.amount} onChange={(e) => update('amount', e.target.value)}/><Field label="Frequency" value={values.frequency} onChange={(e) => update('frequency', e.target.value)}/><Field label="Next payment date" type="date" value={values.next_payment_date} onChange={(e) => update('next_payment_date', e.target.value)}/><Field label="Destination account"><select value={values.destination_account_id || ''} onChange={(e) => update('destination_account_id', e.target.value)}><option value="">No account</option>{data.accounts.map((a) => <option value={a.id} key={a.id}>{a.name}</option>)}</select></Field></>}{edit.type === 'bills' && <><Field label="Name" value={values.name} onChange={(e) => update('name', e.target.value)}/><Field label="Amount" value={values.amount} onChange={(e) => update('amount', e.target.value)}/><Field label="Due date" type="date" value={values.due_date} onChange={(e) => update('due_date', e.target.value)}/><RecurringPaymentFieldsV17 values={values} update={update} accounts={data.accounts} cards={data.cards} categories={data.categories} expenseTypes={data.expenseTypes}/></>}{edit.type === 'recurring' && <><Field label="Name" value={values.name} onChange={(e) => update('name', e.target.value)}/><Field label="Amount" value={values.amount} onChange={(e) => update('amount', e.target.value)}/><Field label="Frequency" value={values.frequency} onChange={(e) => update('frequency', e.target.value)}/><Field label="Next due date" type="date" value={values.next_due_date} onChange={(e) => update('next_due_date', e.target.value)}/><RecurringPaymentFieldsV17 values={values} update={update} accounts={data.accounts} cards={data.cards} categories={data.categories} expenseTypes={data.expenseTypes}/></>}{edit.type === 'planned' && <><Field label="Name" value={values.name} onChange={(e) => update('name', e.target.value)}/><Field label="Estimated amount" value={values.estimated_amount} onChange={(e) => update('estimated_amount', e.target.value)}/><Field label="Planned date" type="date" value={values.planned_date} onChange={(e) => update('planned_date', e.target.value)}/></>}{edit.type === 'budgets' && <><Field label="Name" value={values.name} onChange={(e) => update('name', e.target.value)}/><Field label="Amount" value={values.amount} onChange={(e) => update('amount', e.target.value)}/><Field label="Period" value={values.period} onChange={(e) => update('period', e.target.value)}/></>}{edit.type === 'goals' && <><Field label="Name" value={values.name} onChange={(e) => update('name', e.target.value)}/><Field label="Target amount" value={values.target_amount} onChange={(e) => update('target_amount', e.target.value)}/></>}{edit.type === 'categories' && <><Field label="Name" value={values.name} onChange={(e) => update('name', e.target.value)}/><CategorySelect categories={data.categories} value={values.parent_id} onChange={(value) => update('parent_id', value)}/></>}</div><div className="modal-actions"><button type="button" onClick={() => setEdit(null)}>Cancel</button><button className="primary">Save</button></div></form></div>; }
function ForecastDetailModal({ detail, onClose, onEdit }) { return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal"><div className="modal-head"><h2>{detail.event.name}</h2><button type="button" onClick={onClose}>×</button></div><p>{dateLabel(detail.event.date)} · {money(detail.event.amount)}</p>{detail.record && <button className="primary" onClick={onEdit}>Edit source</button>}</section></div>; }
