...
export default function AppCorrectiveV0174({ authState = null }) {
  ...
  async function loadSupportingData() {
    ...
    setData((current) => {
      ...
      const cachedForecast = forecast ? { ...(current.forecastCache || {}), [rangeDays]: { ...(current.forecastCache?.[rangeDays] || {}), expected: forecast } } : current.forecastCache;
      return { ...current, accounts: accounts || [], cards: cards || [], transactions: transactions || [], income: income || [], recurring: recurring || [], scheduledPayments: resolvedScheduled, paymentAttention: derivedAttention, bills: bills || [], planned: planned || [], categories: categories || [], expenseTypes: referenceData?.expense_types || [], budgets: budgets || [], goals: goals || [], imports: imports || [], review: review || [], suggestions: suggestions || [], insights: insights || [], financialHealth, budgetAnalysis, forecast, paymentPlanning: paymentPlanning === null ? current.paymentPlanning : paymentPlanning, forecastCache: cachedForecast };
    });
  }
  ...
  async function refreshRecurringSlice() {
    ...
    setData((current) => ({ ...current, recurring: recurring || [], forecastCache: {} }));
    try {
      ...
      setData((current) => ({ ...current, scheduledPayments: scheduledPayments || [], paymentAttention, paymentPlanning: paymentPlanning || current.paymentPlanning, forecastCache: {} }));
  ...
  return <div className={`shell ${mobileNavOpen ? 'mobile-nav-open' : ''}`}>
    ...
    <main className="content">
      ...
      {active === 'Overview' && <><Overview data={data} setActive={navigate} rangeDays={rangeDays} setQuick={setQuick} quickDefaults={quickDefaults}/><PaymentsAttentionV17 rows={data.paymentAttention} money={money} dateLabel={dateLabel} onRefresh={loadAll} onOpenAll={() => { localStorage.setItem('fynvo.paymentCentreRequiresAction', 'true'); navigate('Payment Centre'); }}/></>}
...
function DynamicFields({ type, values, set, data, currentId }) {
  ...
  if (type === 'income') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('amount', 'Amount')}{text('next_payment_date', 'Next payment', 'date')}<Field label="Frequency"><select value={values.frequency} onChange={(e) => set('frequency', e.target.value)}><option value="weekly">Weekly</option><option value="fortnightly">Fortnightly</option><option value="monthly">Monthly</option><option value="annual">Annual</option></select></Field><Field label="Destination Account">{accountSelect('destination_account_id')}</Field>{text('payer', 'Payer')}{text('category', 'Category')}</div>;
  if (type === 'accounts') return <div className="form-grid">{text('name', 'Name', 'text', true)}<Field label="Account Type"><select value={values.account_type} onChange={(e) => set('account_type', e.target.value)}>{accountTypeOptions.map(([id, label]) => <option key={id} value={id}>{label}</option>)}{values.account_type && !accountTypeOptions.some(([id]) => id === values.account_type) && <option value={values.account_type}>{accountTypeLabel(values.account_type)} (existing)</option>}</select></Field>{text('opening_balance', 'Opening Balance')}{text('institution', 'Institution')}{text('account_suffix', 'Last 4 digits')}</div>;
  if (type === 'recurring') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('amount', 'Amount')}{text('next_due_date', 'Next due date', 'date')}<Field label="Frequency"><select value={values.frequency} onChange={(e) => set('frequency', e.target.value)}><option value="weekly">Weekly</option><option value="fortnightly">Fortnightly</option><option value="28_days">Every 28 days</option><option value="monthly">Monthly</option><option value="quarterly">Quarterly</option><option value="annual">Annual</option></select></Field><CategorySelect value={values.category_id} onChange={(value) => set('category_id', value)} categories={data.categories}/><Field label="Expense Type"><select value={values.expense_type_id || ''} onChange={(e) => set('expense_type_id', e.target.value || null)}><option value="">Choose Expense Type</option>{data.expenseTypes.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></Field><RecurringPaymentFieldsV17 values={values} set={set} data={data}/>{text('payee_merchant', 'Payee / Merchant')}{text('end_date', 'End Date', 'date')}{text('reminder_days_before', 'Reminder days before')}{text('notes', 'Notes', 'text', false, 'wide')}</div>;
  if (type === 'bills') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('provider', 'Provider')}{text('amount', 'Amount')}{text('due_date', 'Due date', 'date')}<CategorySelect value={values.category_id} onChange={(value) => set('category_id', value)} categories={data.categories}/><RecurringPaymentFieldsV17 values={values} set={set} data={data}/>{text('notes', 'Notes', 'text', false, 'wide')}</div>;
  if (type === 'planned') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('estimated_amount', 'Estimated Amount')}{text('planned_date', 'Planned Date', 'date')}<Field label="Account">{accountSelect('account_id')}</Field>{text('category', 'Category')}<Field label="Priority"><select value={values.priority} onChange={(e) => set('priority', e.target.value)}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></Field></div>;
  if (type === 'categories') return <div className="form-grid">{text('name', 'Name', 'text', true)}<Field label="Parent"><select value={values.parent_id || ''} onChange={(e) => set('parent_id', e.target.value || '')}><option value="">No parent</option>{data.categories.filter((row) => row.is_active !== false && Number(row.id) !== Number(currentId)).map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></Field>{text('icon', 'Icon')}{text('color', 'Colour')}<Field label="Category Type"><select value={values.category_type} onChange={(e) => set('category_type', e.target.value)}><option value="expense">Expense</option><option value="income">Income</option><option value="transfer">Transfer</option></select></Field></div>;
  if (type === 'budgets') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('amount', 'Amount')}<Field label="Period"><select value={values.period} onChange={(e) => set('period', e.target.value)}><option value="weekly">Weekly</option><option value="fortnightly">Fortnightly</option><option value="monthly">Monthly</option><option value="annual">Annual</option></select></Field>{text('category_name', 'Category')}</div>;
  if (type === 'goals') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('target_amount', 'Target Amount')}{text('current_amount', 'Current Amount')}{text('target_date', 'Target Date', 'date')}</div>;
  return null;
}
