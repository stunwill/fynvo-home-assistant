...
export default function AppCorrectiveV0174() {
  ...
  const navigate = (item) => { setActive(item); if (isMobile) { setMobileNavOpen(false); window.requestAnimationFrame(() => window.scrollTo({ top: 0, left: 0, behavior: 'auto' })); } else window.requestAnimationFrame(() => window.scrollTo({ top: 0, left: 0, behavior: 'auto' })); };
...
function DynamicFields({ type, values, set, data, currentId }) {
  ...
  if (type === 'bills') {
    ...
    return <div className="form-grid bill-v112-form">{text('name', 'Name / Description', 'text', true)}{text('amount', 'Amount', 'text', true)}{text('due_date', 'Due date', 'date', true)}{text('payee_merchant', 'Payee / Merchant')}{text('provider', 'Provider')}<Field label="Category"><select value={values.category_id || ''} onChange={(e) => set('category_id', e.target.value || null)}><option value="">Choose Category</option>{(data.categories || []).filter((row) => row.is_active !== false || Number(row.id) === Number(values.category_id)).map((row) => <option key={row.id} value={row.id}>{row.path || row.name}</option>)}</select></Field><Field label="Expense Type"><select value={values.expense_type_id || ''} onChange={(e) => set('expense_type_id', e.target.value || null)}><option value="">Choose Expense Type</option>{(data.expenseTypes || []).filter((row) => row.is_active !== false || Number(row.id) === Number(values.expense_type_id)).map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></Field><Field label="Priority"><select value={values.priority || 'normal'} onChange={(e) => set('priority', e.target.value)}><option value="high">High</option><option value="normal">Normal</option><option value="low">Low</option></select></Field><fieldset className="payment-v17-section wide"><legend>Payment</legend><span className="payment-v17-question">How is this normally paid?</span><div className="payment-v17-handling" role="radiogroup" aria-label="Bill payment handling"><button type="button" role="radio" aria-checked={values.payment_handling === 'automatic'} className={values.payment_handling === 'automatic' ? 'active' : ''} onClick={() => set('payment_handling', 'automatic')}>Paid automatically</button><button type="button" role="radio" aria-checked={values.payment_handling === 'manual'} className={values.payment_handling === 'manual' ? 'active' : ''} onClick={() => set('payment_handling', 'manual')}>I pay this manually</button></div><Field label="Payment Method"><select value={method} onChange={(e) => set('payment_method', e.target.value)}><option value="not_set">Not Set</option><option value="direct_debit">Direct Debit</option><option value="automatic_card_payment">Automatic Card Payment</option><option value="manual_payment">Manual Payment</option><option value="bpay">BPAY</option><option value="bank_transfer">Bank Transfer</option><option value="cash">Cash</option><option value="other">Other</option></select></Field>{method === 'direct_debit' && <Field label="Bank Account">{accountSelect('account_id')}</Field>}{method === 'automatic_card_payment' && <><Field label="Card"><select value={values.card_id || ''} onChange={(e) => set('card_id', e.target.value || null)}><option value="">Choose Card</option>{(data.cards || []).filter((card) => card.is_active !== false).map((card) => <option key={card.id} value={card.id}>{card.display_name || card.name}</option>)}</select></Field>{selectedCard?.account_id && <p className="muted">Funding Account: {data.accounts.find((a) => Number(a.id) === Number(selectedCard.account_id))?.name || 'Linked Account'}</p>}</>}<Field label="Payment handling"><select value={values.payment_handling || 'manual'} onChange={(e) => set('payment_handling', e.target.value)}><option value="manual">Manual</option><option value="automatic">Automatic</option></select></Field>{text('auto_payment_grace_days', 'Auto payment grace days', 'number')}<Field label="Notes" className="wide"><textarea rows="3" value={values.notes || ''} onChange={(e) => set('notes', e.target.value)} /></Field></fieldset></div>;
  }
  if (type === 'accounts') return <div className="form-grid">{text('name', 'Name', 'text', true)}<Field label="Account type"><select value={values.account_type} onChange={(e) => set('account_type', e.target.value)}>{accountTypeOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}{values.account_type && !accountTypeOptions.some(([value]) => value === values.account_type) && <option value={values.account_type}>{accountTypeLabel(values.account_type)} (legacy)</option>}</select></Field>{text('institution', 'Institution')}{text('opening_balance', 'Opening balance')}{text('description', 'Description', 'text', false, 'wide')}</div>;
  if (type === 'planned') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('estimated_amount', 'Estimated amount')}{text('planned_date', 'Planned date', 'date')}<Field label="Priority"><select value={values.priority} onChange={(e) => set('priority', e.target.value)}><option>low</option><option>medium</option><option>high</option></select></Field>{text('category', 'Category')}<Field label="Account">{accountSelect('account_id')}</Field></div>;
  if (type === 'categories') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('icon', 'Icon')}{text('color', 'Color')}<Field label="Parent"><select value={values.parent_id || ''} onChange={(e) => set('parent_id', e.target.value)}><option value="">Top level</option>{data.categories.filter((row) => Number(row.id) !== Number(currentId) && !row.parent_id).map((row) => <option value={row.id} key={row.id}>{row.name}</option>)}</select></Field><Field label="Type"><select value={values.category_type} onChange={(e) => set('category_type', e.target.value)}><option value="expense">Expense</option><option value="income">Income</option><option value="transfer">Transfer</option></select></Field>{text('notes', 'Notes', 'text', false, 'wide')}</div>;
  if (type === 'budgets') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('amount', 'Amount')}<Field label="Period"><select value={values.period} onChange={(e) => set('period', e.target.value)}><option>weekly</option><option>fortnightly</option><option>monthly</option><option>annual</option></select></Field>{text('start_date', 'Start date', 'date')}{text('end_date', 'End date', 'date')}</div>;
  if (type === 'goals') return <div className="form-grid">{text('name', 'Name', 'text', true)}{text('target_amount', 'Target amount')}{text('current_amount', 'Current amount')}{text('start_date', 'Start date', 'date')}{text('target_date', 'Target date', 'date')}<Field label="Priority"><select value={values.priority} onChange={(e) => set('priority', e.target.value)}><option>low</option><option>medium</option><option>high</option></select></Field>{text('description', 'Description', 'text', false, 'wide')}</div>;
  return <div className="form-grid">{Object.keys(values).slice(0, 8).map((key) => text(key, key.replaceAll('_', ' ')))}</div>;
}
