...
function Summary({ summary }) {
  ...
  return <div className="payment-centre-summary" aria-label="Payment summary">
    {items.map(([key, label]) => <article key={key} className={`payment-centre-summary-card summary-${key}`}>
      <small>{label}</small>
      <strong>{money(summary?.[key]?.amount)}</strong>
      <span>{summary?.[key]?.count || 0} item{summary?.[key]?.count === 1 ? '' : 's'}</span>
    </article>)}
  </div>;
}
...
function PaymentRow({ row, onOpen, onAction }) {
  ...
  return <article className={`payment-centre-row ${paymentNeedsAction(row) ? 'needs-action' : ''}`}>
    <button type="button" className="payment-centre-row-main" onClick={() => onOpen(row)}>
      ...
      <span className="payment-centre-name"><strong>{row.name}</strong><small>{row.category || 'Uncategorised'} · {paymentSourceLabel(row)}</small>{reason && <small>{reason}</small>}</span>
...
function DetailModal({ detail, onClose, onMarkPaid, onSkip, onReview, onViewTransaction, onEditBill, onCancelBill, onOpenRecurring }) {
  ...
  return <div className="payment-centre-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="payment-centre-detail" role="dialog" aria-modal="true" aria-label={`${detail.name} payment detail`}><header><div><StatusBadge status={detail.status}/><h2>{detail.name}</h2><p>{paymentSourceLabel(detail)}</p></div><button type="button" onClick={onClose} aria-label="Close payment detail">×</button></header><div className="payment-centre-detail-body">
    ...
    {detail.source_type === 'bill' && unresolved && <button type="button" onClick={() => onEditBill(detail)}>Edit Bill</button>}
    {detail.source_type === 'bill' && unresolved && <button type="button" onClick={() => onCancelBill(detail)}>Cancel Bill</button>}
    {detail.source_type === 'scheduled_payment' && unresolved && <button type="button" onClick={() => onSkip(detail)}>Skip payment</button>}
    {detail.source_type === 'scheduled_payment' && detail.recurring_expense_id && <button type="button" onClick={() => onOpenRecurring(detail)}>Open Recurring Expense</button>}
    {canReviewMatch && <button type="button" onClick={() => onReview(detail)}>Review Match</button>}
    {(detail.payment_handling === 'manual' || ['due', 'due_today', 'overdue'].includes(detail.status)) && unresolved && <button type="button" className="primary" onClick={() => onMarkPaid(detail)}>Mark as paid</button>}
...
function MarkPaidModal({ payment, onClose, onSaved }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setError(requestError.message || 'Could not mark this payment paid.');
    } finally {
      setSaving(false);
    }
  };
...
export default function PaymentCentreV112({ data, onNavigate, onRefreshSupporting, onEditBill, onOpenRecurring }) {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      setResult(await apiRequest(query));
    } catch (requestError) {
      setError(requestError.message || 'Could not load the Payment Centre.');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const timer = window.setTimeout(load, filters.search ? 180 : 0);
  ...
  const open = async (row) => {
    setActionError('');
    try {
      setDetail(await apiRequest(`/payment-centre/${row.source_type}/${row.id}`));
    } catch (requestError) {
      setActionError(requestError.message || 'Could not load payment detail.');
    }
  };
  ...
  const cancelBill = async (row) => {
    setActionError('');
    try {
      await apiRequest(`/bills/${row.id}/cancel`, { method: 'POST', body: JSON.stringify({ version: row.version }) });
      await refreshed();
    } catch (requestError) {
      setActionError(requestError.message || 'Could not cancel this Bill.');
    }
  };
  ...
  const skip = async (row) => {
    setActionError('');
    try {
      if (row.source_type !== 'scheduled_payment') return;
      await apiRequest(`/scheduled-payments/${row.id}/skip`, { method: 'POST', body: JSON.stringify({ note: 'Skipped from Payment Centre' }) });
      await refreshed();
    } catch (requestError) {
      setActionError(requestError.message || 'Could not skip this payment.');
    }
  };
  ...
  const editBill = (row) => {
    setDetail(null);
    setActionError('');
    if (onEditBill) onEditBill(row);
    else onNavigate('Bills');
  };

  const openRecurring = (row) => {
    setDetail(null);
    setActionError('');
    if (onOpenRecurring) onOpenRecurring(row);
    else onNavigate('Recurring Expenses');
  };

  const action = (row, kind) => {
    setActionError('');
    if (kind === 'mark_paid') { setDetail(null); setMarkingPaid(row); }
    else if (kind === 'review' && row.source_type === 'scheduled_payment') onNavigate('Review Queue');
    else if (kind === 'review') onNavigate('Transactions');
    else if (kind === 'edit') editBill(row);
    else if (kind === 'cancel') cancelBill(row);
    else if (kind === 'open_recurring') openRecurring(row);
    else open(row);
  };

  const groups = groupPayments(result?.rows || []);
  return <section className="payment-centre-page">
    <div className="payment-centre-head"><div><h1>Payment Centre</h1><p>What needs to be paid, what will be paid automatically, and what needs review.</p></div><button type="button" className="primary ghost" onClick={() => onNavigate('Bills')}>+ Add Bill</button></div>
    ...
    {actionError && <div className="payment-centre-action-error" role="alert">{actionError}</div>}
    ...
    <DetailModal
      ...
      onMarkPaid={(row) => { setDetail(null); setMarkingPaid(row); }}
      onSkip={skip}
      onReview={() => { setDetail(null); onNavigate('Review Queue'); }}
      onViewTransaction={() => { setDetail(null); onNavigate('Transactions'); }}
      onEditBill={editBill}
      onCancelBill={cancelBill}
      onOpenRecurring={openRecurring}
    />
    <MarkPaidModal payment={markingPaid} onClose={() => { setMarkingPaid(null); setActionError(''); }} onSaved={refreshed}/>
  </section>;
}
