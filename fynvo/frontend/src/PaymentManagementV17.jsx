...
export function PaymentsAttentionV17({ rows, money, dateLabel, onRefresh }) {
  const [paying, setPaying] = useState(null); const [reviewing, setReviewing] = useState(null); const [rescheduling, setRescheduling] = useState(null);
  ...
  const changeDate = (row) => { setReviewing(null); setPaying(null); setRescheduling(row); };
  return <article className="panel payments-v17-attention"><div className="panel-head compact"><div><h2>Payments requiring attention</h2><small>{rows.length ? `${rows.length} payment${rows.length === 1 ? '' : 's'} need review` : 'Nothing needs attention'}</small></div></div>{rows.length ? rows.slice(0, 6).map((row) => <div className="payments-v17-row" key={row.id}><div><PaymentStatusBadge status={row.status}/><strong>{row.name}</strong><small>{row.payment_method_label}{row.account_name ? ` · ${row.account_name}` : ''}</small></div><div><strong>{money(row.expected_amount) || 'Not set'}</strong><small>{dateLabel(row.expected_date)}</small>{row.is_date_overridden && <small>Usual {dateLabel(row.original_expected_date || row.occurrence_date)}</small>}</div><div className="payment-v17-attention-actions"><button type="button" onClick={() => changeDate(row)}>Change payment date</button>{row.payment_handling === 'manual' ? <button onClick={() => { setPaying(row); setForm({ paid_date: new Date().toISOString().slice(0, 10), paid_amount: row.expected_amount || '', note: '' }); }}>Mark paid</button> : <button type="button" onClick={() => setReviewing(row)}>Review</button>}</div></div>) : <p className="muted">No overdue, due or unconfirmed automatic payments.</p>}
