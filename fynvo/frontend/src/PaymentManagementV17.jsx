...
const statusLabels = {
  upcoming: 'Upcoming', due: 'Requires payment', due_today: 'Due today', overdue: 'Overdue', expected_automatically: 'Expected automatically',
...
export function RecurringPaymentFieldsV17({ values, set, data }) {
  ...
  const setMethod = (next) => set('payment_method', next);
  return <fieldset className="payment-v17-section"><legend>Payment</legend>
    ...
    {values.payment_handling === 'automatic' && <label className="field"><span>Payment confirmation period</span><select value={values.auto_payment_grace_days ?? 3} onChange={(event) => set('auto_payment_grace_days', Number(event.target.value))}><option value={1}>1 day</option><option value={2}>2 days</option><option value={3}>3 days</option><option value={5}>5 days</option><option value={7}>7 days</option></select><small className="payment-v17-linked">How long Fynvo should wait after the due date for an automatic payment to be confirmed. This does not change the due date.</small></label>}
  </fieldset>;
}

export function CardsPageV17({ accounts, cards, onRefresh }) {
  const [edit, setEdit] = useState(null); const [error, setError] = useState(''); const [localCards, setLocalCards] = useState(cards);
  ...
  const save = async (event) => { event.preventDefault(); setError(''); const response = await api(edit.id ? `/cards/${edit.id}` : '/cards', { method: edit.id ? 'PUT' : 'POST', body: JSON.stringify(edit) }); if (!response.ok) { const payload = await response.json().catch(() => null); setError(payload?.detail || 'Could not save Card.'); return; } const saved = await response.json(); setLocalCards((current) => edit.id ? current.map((card) => Number(card.id) === Number(saved.id) ? saved : card) : [...current, saved]); setEdit(null); await onRefresh(); };
  const grouped = useMemo(() => accounts.map((account) => ({ account, cards: localCards.filter((card) => Number(card.account_id) === Number(account.id)) })), [accounts, localCards]);
...
function attentionReason(row) { return row.attention_reason || statusLabels[row.status] || 'Review payment'; }

function AttentionRescheduleModal({ payment, money, dateLabel, onClose, onSaved }) {
  ...
  const save = async (event) => {
    ...
    try {
      const response = await api(`/scheduled-payments/${payment.id}/reschedule`, { method: 'POST', body: JSON.stringify({ expected_date: expectedDate, reason: reason || null, note: note || null, version: payment.version }) });
      if (!response.ok) { const payload = await response.json().catch(() => null); throw new Error(payload?.detail || 'Could not change this payment date.'); }
      await onSaved();
    } catch (requestError) { setError(requestError.message || 'Could not change this payment date.'); }
    finally { setSaving(false); }
  };
  return <div className="modal-backdrop payment-v17-reschedule-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="modal payment-v17-reschedule-modal" onSubmit={save}><div className="panel-head"><div><h2>Change payment date</h2><p>{payment.name} · {money(payment.expected_amount)}</p></div><button type="button" onClick={onClose}>×</button></div><p className="muted payment-v17-reschedule-helper">This changes only this payment. Your recurring schedule will not be changed.</p><div className="detail-grid"><div className="detail-item"><span>Usual payment date</span><strong>{dateLabel(payment.original_expected_date || payment.occurrence_date || payment.expected_date)}</strong></div><div className="detail-item"><span>Expected this time</span><strong>{dateLabel(expectedDate)}</strong></div></div><div className="form-grid"><label className="field"><span>New expected date</span><input required type="date" value={expectedDate} onChange={(event) => setExpectedDate(event.target.value)}/></label><label className="field"><span>Reason (optional)</span><select value={reason} onChange={(event) => setReason(event.target.value)}><option value="">No reason selected</option><option>Payment failed</option><option>Payment date changed by provider</option><option>Insufficient funds</option><option>Payment deferred</option><option>User requested different date</option><option>Other</option></select></label><label className="field wide"><span>Note (optional)</span><textarea rows="3" value={note} onChange={(event) => setNote(event.target.value)}/></label></div>{error && <p className="error">{error}</p>}<div className="modal-actions"><button type="button" onClick={onClose}>Cancel</button><button className="primary" disabled={saving}>{saving ? 'Saving…' : 'Save date'}</button></div></form></div>;
}

export function PaymentsAttentionV17({ rows, money, dateLabel, onRefresh, onOpenAll = () => {} }) {
  const [paying, setPaying] = useState(null); const [reviewing, setReviewing] = useState(null); const [rescheduling, setRescheduling] = useState(null); const [menu, setMenu] = useState(null); const [error, setError] = useState('');
  const [form, setForm] = useState({ paid_date: new Date().toISOString().slice(0, 10), paid_amount: '', note: '' });
  useEffect(() => { if (menu === null) return undefined; const close = (event) => { if (!event.target.closest('.payments-attention-v122-menu')) setMenu(null); }; const escape = (event) => { if (event.key === 'Escape') setMenu(null); }; document.addEventListener('click', close); document.addEventListener('keydown', escape); return () => { document.removeEventListener('click', close); document.removeEventListener('keydown', escape); }; }, [menu]);
  const openPay = (row) => { setMenu(null); setError(''); setPaying(row); setForm({ paid_date: new Date().toISOString().slice(0, 10), paid_amount: row.expected_amount || '', note: '' }); };
  const markPaid = async (event) => { event.preventDefault(); setError(''); const response = await api(`/scheduled-payments/${paying.id}/mark-paid`, { method: 'POST', body: JSON.stringify({ ...form, paid_amount: form.paid_amount || paying.expected_amount }) }); if (response.ok) { setPaying(null); await onRefresh(); } else setError((await response.json().catch(() => null))?.detail || 'Could not mark this payment as paid.'); };
  const skip = async () => { if (!paying) return; setError(''); const response = await api(`/scheduled-payments/${paying.id}/skip`, { method: 'POST', body: JSON.stringify({ note: form.note || 'Skipped by user' }) }); if (response.ok) { setPaying(null); await onRefresh(); } else setError((await response.json().catch(() => null))?.detail || 'Could not skip this payment.'); };
  const changeDate = (row) => { setMenu(null); setReviewing(null); setPaying(null); setRescheduling(row); };
  const review = (row) => { setMenu(null); setReviewing(row); };
  return <article className="panel payments-v17-attention payments-attention-v122" onClick={(event) => { if (event.target === event.currentTarget) setMenu(null); }}><div className="payments-attention-v122-head"><div className="payments-attention-v122-title"><span className="payments-attention-v122-icon" aria-hidden="true">♧</span><div><h2>Payments requiring attention</h2><small>{rows.length ? `${rows.length} payment${rows.length === 1 ? '' : 's'} need review` : 'No payments need attention'}</small></div></div><button type="button" className="payments-attention-v122-see-all" onClick={onOpenAll}>See all <span aria-hidden="true">›</span></button></div>{rows.length ? <div className="payments-attention-v122-list">{rows.slice(0, 6).map((row) => { const reason = attentionReason(row); const overdue = row.status === 'overdue'; const method = row.payment_method_label || row.payment_method || 'Not set'; const context = [row.category || row.category_name, row.subcategory || row.expense_type].filter(Boolean); return <article className={`payments-attention-v122-card ${overdue ? 'is-overdue' : 'is-warning'}`} key={row.id}><div className="payments-attention-v122-main"><div className="payments-attention-v122-status"><span aria-hidden="true">{overdue ? '◷' : '!'}</span>{reason}</div><strong className="payments-attention-v122-name">{row.name || 'Payment'}</strong><span className="payments-attention-v122-method">{method}{row.account_name ? ` · ${row.account_name}` : row.card_name ? ` · ${row.card_name}` : ''}</span>{context.length > 0 && <div className="payments-attention-v122-chips">{context.slice(0, 2).map((value) => <span key={value}>{value}</span>)}</div>}</div><div className="payments-attention-v122-amount"><strong>{money(row.expected_amount) || 'Not set'}</strong><span>{dateLabel(row.expected_date)}</span><small className={overdue ? 'overdue' : ''}>{attentionTiming(row.expected_date)}</small></div><div className="payments-attention-v122-menu"><button type="button" aria-label={`More actions for ${row.name || 'payment'}`} aria-expanded={menu === row.id} onClick={() => setMenu(menu === row.id ? null : row.id)}>⋮</button>{menu === row.id && <div className="payments-attention-v122-menu-popover" role="menu"><button type="button" role="menuitem" onClick={() => changeDate(row)}>Change payment date</button><button type="button" role="menuitem" onClick={() => openPay(row)}>Skip payment</button></div>}</div><div className="payments-attention-v122-actions"><button type="button" onClick={() => changeDate(row)}>Change payment date</button>{row.payment_handling === 'manual' ? <button type="button" className="positive-action" onClick={() => openPay(row)}>Mark as paid</button> : <button type="button" className="review-action" onClick={() => review(row)}>Review</button>}</div></article>; })}</div> : null}<button type="button" className="payments-attention-v122-view-all" onClick={onOpenAll}>View all {rows.length} payment{rows.length === 1 ? '' : 's'} <span aria-hidden="true">›</span></button>
    {paying && <div className="modal-backdrop"><form className="modal" onSubmit={markPaid}><div className="panel-head"><div><h2>Mark as paid</h2><p>{paying.name} · {money(paying.expected_amount)}</p></div><button type="button" aria-label="Close mark as paid dialog" onClick={() => setPaying(null)}>×</button></div><div className="form-grid"><label className="field"><span>Paid date</span><input type="date" value={form.paid_date} onChange={(event) => setForm({ ...form, paid_date: event.target.value })}/></label><label className="field"><span>Paid amount</span><input value={form.paid_amount} onChange={(event) => setForm({ ...form, paid_amount: event.target.value })}/></label><label className="field wide"><span>Note (optional)</span><input value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })}/></label></div><p className="muted">If this payment did not occur, use Skip payment instead of recording a $0 payment.</p>{error && <p className="error" role="alert">{error}</p>}<div className="modal-actions"><button type="button" className="danger-ghost" onClick={skip}>Skip payment</button><button type="button" onClick={() => setPaying(null)}>Cancel</button><button className="primary">Mark as paid</button></div></form></div>}
    {reviewing && <div className="modal-backdrop"><section className="modal"><div className="panel-head"><div><h2>{attentionReason(reviewing)}</h2><p>{reviewing.name}</p></div><button type="button" aria-label="Close payment review dialog" onClick={() => setReviewing(null)}>×</button></div><div className="detail-grid"><div className="detail-item"><span>Expected amount</span><strong>{money(reviewing.expected_amount)}</strong></div><div className="detail-item"><span>Expected date</span><strong>{dateLabel(reviewing.expected_date)}</strong></div><div className="detail-item"><span>Payment Method</span><strong>{reviewing.payment_method_label || 'Not set'}</strong></div><div className="detail-item"><span>Payment source</span><strong>{reviewing.card_name || reviewing.account_name || 'Not set'}</strong></div></div><p className="muted">Review the matching transaction before confirming this automatic payment. Fynvo will not mark it paid without confirmation.</p><div className="modal-actions"><button type="button" onClick={() => changeDate(reviewing)}>Change payment date</button><button type="button" onClick={() => setReviewing(null)}>Close</button></div></section></div>}
    {rescheduling && <AttentionRescheduleModal payment={rescheduling} money={money} dateLabel={dateLabel} onClose={() => setRescheduling(null)} onSaved={async () => { setRescheduling(null); await onRefresh(); }}/>} 
  </article>;
}
