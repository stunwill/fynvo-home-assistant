...
export function summarisePayments(rows, referenceDate = new Date()) {
  ...
  const boundary = localDateKey(nextMonth);
  const bucketRows = [
...
function PaymentActions({ payment, onEdit, normaliseRecord, onRefresh, money, dateLabel }) {
  ...
  const [form, setForm] = useState({ paid_date: localDateKey(), paid_amount: payment.expected_amount || '', note: '' });
