...
export default function CashPlanPageV121() {
  ...
  const incomplete = safePlan?.incomplete || !safePlan?.safe_to_spend && safePlan?.safe_to_spend !== '0.00';
  ...
  return <section className="cash-plan-v121" aria-label="Cash Plan">
    ...
    {!loading && !error && <>
      <article className={`cash-plan-v121-hero ${Number(safeValue) < 0 ? 'negative' : ''}`}><span>{Number(safeValue) < 0 ? 'Projected shortfall' : 'Safe to spend'}</span><strong>{money(safeValue)}</strong><p>{safePlan?.planning_end ? `Until ${dateLabel(safePlan.planning_end)}` : safePlan?.warnings?.join(' ') || 'A planning horizon needs configuration.'}</p></article>
      ...
      {incomplete && <div className="cash-plan-v121-note" role="status">{safePlan?.warnings?.join(' ') || payCycle?.completeness?.message || 'Some planning information is incomplete. Known payments remain available below.'}</div>}
