...
function activateNavigation(label) {
  ...
  const buttons = [
    ...document.querySelectorAll(
      "#fynvo-navigation button, .sidebar button, .nav-group button, main.content button",
    ),
  ];
...
export default function MobileOverviewV1240({
  authenticated = false,
  productionVersion = "",
}) {
  ...
  useEffect(() => {
    if (!active || !isOverview) {
      document.body.classList.remove("fynvo-overview-v1240-active");
      setHost(null);
      return undefined;
    }
    ...
    setHost(node);
    document.body.classList.add("fynvo-overview-v1240-active");
    return () => {
      document.body.classList.remove("fynvo-overview-v1240-active");
      node.remove();
      setHost(null);
  ...
  const model = useMemo(() => {
    ...
    return {
      ...
      planMessage: pressure
        ? `${pressure.name || "An upcoming payment"} is the next pressure point.`
        : "No immediate pressure point is identified.",
      warning:
  ...
  const overview =
    isOverview && host
      ? createPortal(
          <section className="fynvo-overview-v1240" aria-label="Fynvo Overview">
            {error && (
            ...
            <section
              className={`fynvo-ui-card fynvo-overview-v1240-safe ${model.safeAvailable ? "available" : "unavailable"}`}
              aria-labelledby="safe-to-spend-title"
            >
              <div className="fynvo-overview-v1240-card-head">
                <div>
              ...
              {model.progress !== null && (
                <div
                  ...
                  role="progressbar"
                  aria-label="Safe to spend position"
            ...
            <section
              className="fynvo-ui-card fynvo-overview-v1240-section fynvo-overview-v1240-allocation"
              aria-labelledby="allocation-title"
            >
              <div className="fynvo-ui-section-head">
                <div>
                  <h2 id="allocation-title">Payday allocation</h2>
                  <p>
              ...
              <strong className="fynvo-overview-v1240-plan-value">
                {model.allocation?.total_recommended_allocation == null
                  ? "Unavailable"
                  : `${money(model.allocation.total_recommended_allocation)} to allocate`}
              </strong>
              <p>
                {model.allocation?.status === "ready"
                  ? model.allocationNeedCount
            ...
            <section
              className="fynvo-ui-card fynvo-overview-v1240-section fynvo-overview-v1240-attention"
                ...
                <div>
                  ...
                  <p>
                    {model.attentionCount
                      ? `${model.attentionCount} payment${model.attentionCount === 1 ? "" : "s"} require your attention`
                      : "All good"}
                  </p>
                </div>
            ...
            >
                ...
                <div>
                  ...
                  <p>
                    {model.upcoming.length
                      ? `Next ${model.upcoming.length} payment${model.upcoming.length === 1 ? "" : "s"}`
              ...
              {loading && !planning ? (
                ...
                <p className="fynvo-ui-state">You’re all caught up.</p>
              )}
            </section>

            <section
              className="fynvo-ui-card fynvo-overview-v1240-section fynvo-overview-v1240-plan"
