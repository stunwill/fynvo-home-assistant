...
function FilterBar({ filters, setFilters, accounts, categories, search, setSearch }) {
  ...
  const activeSecondaryFilters = ['account', 'category', 'type', 'reconciliation', 'source'].filter((key) => filters[key] !== DEFAULT_FILTERS[key]).length;
  const clear = () => { setFilters(DEFAULT_FILTERS); setSearch(''); };
  const secondaryFilters = <div className="transaction-secondary-filter-grid">
  ...
  return <>
    <div className="transaction-filters">
      <input aria-label="Search transactions" placeholder="Search transactions…" value={search} onChange={(event) => setSearch(event.target.value)}/>
      ...
      <div className="transaction-desktop-secondary-filters">{secondaryFilters}</div>
      <button type="button" className="transaction-mobile-filter-trigger" onClick={() => setMobileFiltersOpen(true)}>Filters{activeSecondaryFilters ? ` (${activeSecondaryFilters})` : ''}</button>
      <button type="button" disabled={JSON.stringify(filters) === JSON.stringify(DEFAULT_FILTERS) && !search} onClick={clear}>Clear filters</button>
    </div>
    {mobileFiltersOpen && <div className="transaction-mobile-filter-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setMobileFiltersOpen(false)}><section className="transaction-mobile-filter-sheet" role="dialog" aria-modal="true" aria-label="Transaction filters"><header><strong>Filters{activeSecondaryFilters ? ` (${activeSecondaryFilters})` : ''}</strong><button type="button" aria-label="Close filters" onClick={() => setMobileFiltersOpen(false)}>×</button></header>{secondaryFilters}<div className="modal-actions"><button type="button" onClick={clear}>Clear filters</button><button type="button" className="primary" onClick={() => setMobileFiltersOpen(false)}>Apply</button></div></section></div>}
  </>;
}
...
export default function TransactionWorkspace({ accounts = [], categories = [], money, dateLabel, refreshKey }) {
  ...
  return <section className="transaction-workspace"><div className="panel-head transaction-workspace-head"><div><h2>Transactions</h2><p>Review actual money movements, categorise them and reconcile expected payments.</p></div><span>{filtered.length} shown · {rows.length} loaded</span></div>
    ...
    {loadState === 'loading' && !rows.length ? <div className="panel transaction-load-state" role="status">Loading Transactions…</div> : loadState === 'error' && !rows.length ? <div className="panel transaction-load-state"><strong>Could not load Transactions</strong><p>{error}</p><button className="primary" type="button" onClick={load}>Retry</button></div> : <>
      <div className="transaction-summary-grid"><SummaryCard label="Money in" value={money(moneyIn)} hint={`${nonTransfers.filter((row) => row.transaction_type === 'income').length} transactions`}/><SummaryCard label="Money out" value={money(-moneyOut)} hint={`${nonTransfers.filter((row) => row.transaction_type === 'expense').length} transactions`}/><SummaryCard label="Unmatched" value={unmatched} hint={candidateState === 'error' ? 'Match suggestions unavailable' : 'Transactions to reconcile'}/><SummaryCard label="Uncategorised" value={uncategorised} hint="Needs a Category"/></div>
      <div className="transaction-table"><div className="transaction-table-head"><button type="button" onClick={() => toggleSort('date')}>Date</button><button type="button" onClick={() => toggleSort('merchant')}>Description</button><button type="button" onClick={() => toggleSort('account')}>Account</button><button type="button" onClick={() => toggleSort('category')}>Category</button><button type="button" onClick={() => toggleSort('amount')}>Amount</button><span>Status</span></div>{filtered.map((row) => <button type="button" className="transaction-row" key={row.id} onClick={() => setSelectedId(row.id)}><time>{dateLabel(row.date)}</time><span><strong>{row.merchant || row.description}</strong><small>{row.description}</small></span><span>{row.account_name || 'Unknown account'}</span><span>{categoryById.get(Number(row.category_id))?.path || displayCategory(row)}</span><strong className={row.transaction_type === 'income' ? 'positive' : row.transaction_type === 'expense' ? 'negative' : ''}>{money(row.amount)}</strong><span className={`transaction-status ${row.reconciliation_status || 'unmatched'}`}>{reconciliationLabel(row.reconciliation_status)}</span></button>)}</div>
      {!filtered.length && <div className="empty"><strong>No Transactions match these filters</strong><p>Clear or adjust filters to see more activity.</p></div>}
    </>}
    {selected && <TransactionDetail row={selected} categories={categories} candidates={candidates} money={money} dateLabel={dateLabel} onClose={() => setSelectedId(null)} onChanged={async () => { await refresh(); setSelectedId(null); }}/>} 
  </section>;
}
