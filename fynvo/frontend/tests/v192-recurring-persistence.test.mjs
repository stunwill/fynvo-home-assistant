...
test('new account choices are limited to Transaction Account and Cash while legacy labels remain preserved', () => {
  const optionsStart = app.indexOf('const accountTypeOptions');
  const legacyStart = app.indexOf('const legacyAccountTypeLabels', optionsStart);
  const optionsBlock = app.slice(optionsStart, legacyStart);
  assert.match(optionsBlock, /Transaction Account/);
  assert.match(optionsBlock, /Cash/);
  for (const removed of ['Savings Account', 'Offset Account', 'Credit Card', 'Mortgage', 'Personal Loan', 'Car Loan', 'Line of Credit', 'Investment Account', 'Superannuation', 'Other Asset', 'Other Liability']) {
    assert.doesNotMatch(optionsBlock, new RegExp(removed));
  }
  assert.match(app, /const legacyAccountTypeLabels/);
  assert.match(app, /Savings Account/);
